...
async function getAccounting() : Promise<any>{ 
    try{
        const loanScheduleAccrualData = await retrieveScheduleAccrualData();
        if(!Array.isArray(loanScheduleAccrualData)){
            console.error("Error in getAccounting: loanScheduleAccrualData is not an array", loanScheduleAccrualData);
            return [];            
        }
        console.error("loanScheduleAccrualData:", loanScheduleAccrualData);
        const accounting = loanScheduleAccrualData.map((item) => {
            const {loanId, officeId} = item;
            return {
                loanId,
                officeId,
                duedate,
